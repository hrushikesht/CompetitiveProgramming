...
int main()
{
    ...
    if(sum==2) swap(hash[1],hash[2]);

    if(hash[1].size()>=1){
    	string temp=s;
    	temp.erase(temp.begin()+hash[1].back());
    	snip(temp);
    ...
    if(hash[2].size()>=2){
    	string temp=s;
    	temp.erase(temp.begin()+hash[2].back());
    	temp.erase(temp.begin()+hash[2].at(hash[2].size()-2));
    	snip(temp);
